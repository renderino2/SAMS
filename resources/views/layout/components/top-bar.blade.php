...
<div class="top-bar-boxed h-[70px] z-[51] relative border-b border-white/[0.08] mt-12 md:-mt-5 -mx-3 sm:-mx-8 px-3 sm:px-8 md:pt-0 mb-12">
    <div class="h-full flex items-center">
        ...
        <a href="#" class="-intro-x hidden md:flex items-center">
            <img alt="Midone - HTML Admin Template" class="w-10" data-action="zoom" src="{{ asset('build/assets/images/final-cjc-logo-1.png') }}">
            ...
            <div class="notification-content pt-2 dropdown-menu">
                <div class="notification-content__box dropdown-content">
                    <div class="notification-content__title">Notifications
                        <hr class="dropdown-divider border-white/[0.08]"><hr>
                    </div>
        ...
        <div class="intro-x dropdown w-8 h-8">
            <div class="dropdown-toggle w-8 h-8 rounded-full overflow-hidden shadow-lg image-fit zoom-in scale-110" role="button" aria-expanded="false" data-tw-toggle="dropdown">
                <img alt="{{ optional(Auth::user())->full_name ?? optional(Auth::user())->name }}" 
                     src="{{ Auth::user() && Auth::user()->profile_photo ? asset('storage/profiles/' . Auth::user()->profile_photo) : asset('build/assets/images/placeholders/200x200.jpg') }}">
            </div>
            <div class="dropdown-menu w-56">
                <ul class="dropdown-content bg-primary/80 before:block before:absolute before:bg-black before:inset-0 before:rounded-md before:z-[-1] text-white">
                    <li class="p-2">
                        <div class="font-medium">{{ optional(Auth::user())->full_name ?? optional(Auth::user())->name }}</div>
                        <div class="text-xs text-white/60 mt-0.5 dark:text-slate-500">{{ optional(Auth::user())->role }}</div>
                    </li>
